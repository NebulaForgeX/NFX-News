...
if nfxnews_postgres_use_docker && [[ -z "${POSTGRES_CONTAINER_NAME:-}" ]]; then
...
ensure_postgresql_running() {
  ...
  if nfxnews_postgres_use_docker; then
    ...
    if ! nfxnews_psql_admin -c "SELECT 1" >/dev/null 2>&1; then
...
clean_shadow_connections() {
  ...
  nfxnews_psql_admin -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${POSTGRES_DB_SHADOW}';" >/dev/null 2>&1
  sleep 2
  nfxnews_psql_admin -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${POSTGRES_DB_SHADOW}';" >/dev/null 2>&1
}
...
clean_diff_database() {
  ...
  clean_shadow_connections
  result=$(nfxnews_psql_admin -tc "SELECT 1 FROM pg_database WHERE datname = '${POSTGRES_DB_SHADOW}'" 2>&1)
  if echo "${result}" | grep -qE '\s*1\s*'; then
    echo "Dropping existing shadow database..."
    nfxnews_psql_admin -c "DROP DATABASE ${POSTGRES_DB_SHADOW};" >/dev/null 2>&1
  fi
  echo "Creating shadow database..."
  create_result=$(nfxnews_psql_admin -c "CREATE DATABASE ${POSTGRES_DB_SHADOW};" 2>&1)
  if [ $? -ne 0 ] && ! echo "${create_result}" | grep -qi "already exists"; then
    echo "Warning: Failed to create shadow database: ${create_result}"
  fi
  target_result=$(nfxnews_psql_admin -tc "SELECT 1 FROM pg_database WHERE datname = '${TARGET_DB}'" 2>&1)
  if ! echo "${target_result}" | grep -qE '\s*1\s*'; then
    echo "Creating target database: ${TARGET_DB}"
    create_target_result=$(nfxnews_psql_admin -c "CREATE DATABASE ${TARGET_DB};" 2>&1)
