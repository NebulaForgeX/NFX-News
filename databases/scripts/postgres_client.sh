...
nfxnews_effective_pgsslmode() {
  local s
  s=$(nfxnews_normalize_pgsslmode)
  if [[ -n "$s" ]]; then
    printf '%s' "$s"
    return
  fi
...
nfxnews_psql_admin() {
  if nfxnews_postgres_use_docker; then
    ...
    nfxnews_docker exec "${POSTGRES_CONTAINER_NAME}" psql -U "${POSTGRES_USER}" -d postgres "$@"
  else
    nfxnews_ensure_psql_cli || return 1
    export PGPASSWORD="${POSTGRES_PASSWORD}"
    export PGSSLMODE="$(nfxnews_effective_pgsslmode)"
