...
const MCPPage = memo(() => {
  const { t } = useTranslation("MCPPage");
  ...
  return (
    <PageFrame>
      <PageHeader icon={Wand2} title={t("title")} description={t("subtitle")} />
      <Flex direction="column" gap="4">
        <Card>
          <CardHeader icon={<Wand2 size={18} />} title={t("tools")} />
          {(tools ?? []).length === 0 ? (
            <EmptyState icon={Wand2} title={t("emptyTools")} />
          ) : (
        ...
        <Card>
          <CardHeader icon={<Wand2 size={18} />} title={t("run")} />
